...
# 인자 확인
if [ "$#" -ne 4 ]; then
    echo "Usage: $0 molecule method ns step" # pace height sigma
    exit 1
fi
...
for seed in 0 1 2 3; do
    ...
    if [ "${method}" != "ref" ] && [ "${method}" != "phi" ]; then
        cp "./simulations/${molecule}/${method}/${ns}/test.pt" "${base_dir}/test.pt"
    fi

    # sed -i "0,/PACE=/{s|PACE=.*|PACE=${pace}|}" ${base_dir}/plumed.dat
    # sed -i "0,/HEIGHT=/{s|HEIGHT=.*|HEIGHT=${height}|}" ${base_dir}/plumed.dat
    sed -i "s|FILE=|FILE=${base_dir}/|g" ${base_dir}/plumed.dat

    # if [ "${method}" == "ref" ]; then
    #     sed -i "0,/SIGMA=/{s|SIGMA=.*|SIGMA=${sigma},${sigma}|}" ${base_dir}/plumed.dat
    # else
    #     sed -i "0,/SIGMA=/{s|SIGMA=.*|SIGMA=${sigma}|}" ${base_dir}/plumed.dat
    # fi
